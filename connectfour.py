...
class ConnectFourBoard(object):
    ...
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1, k_value = 4):
    ...
    def get_k_value(self):
        """ Return the k value of the game """
        return self.k_value
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(), k_value = self.k_value)
    ...
    def is_win(self):
        ...
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                ...
                if cell_player != 0:
                    win = self._is_win_from_cell(i,j, self.k_value)
                    if win:
                        self._is_win = win
                        return cell_player
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(), k_value = self.k_value)
...
def run_game(player1, player2, board = ConnectFourBoard(k_value = 4)): # Pass new value of k
